#!/usr/bin/env python3
"""Cornell Med School Finder — finds Cornell undergrad alumni at top medical schools.
...
DESCRIPTION = (
    "Research project tracking Cornell University undergraduate alumni who went on "
    "to attend top-20 medical schools (including Ivy League med schools like Harvard "
    "Medical School, Columbia Vagelos, Penn Perelman, Yale School of Medicine, "
    "Dartmouth Geisel, and other top programs like Johns Hopkins, Stanford, "
    "UCSF, Duke, WashU, UChicago Pritzker, NYU Grossman, Baylor, Northwestern Feinberg, "
    "UCLA Geffen, UMich, Vanderbilt, UCSD, Sinai Icahn, and Emory). Goal: identify their "
    "undergraduate backgrounds, majors, extracurriculars, research experience, and "
    "career trajectories."
)

TARGET = (
    "Cornell University undergraduate alumni who matriculated at top-20 US medical "
    "schools. Focus on LinkedIn profiles, Cornell alumni directories, medical school "
    "class profiles, and published interviews."
)

DETAILS = (
    "Collecting profiles of Cornell undergrads at top med schools to understand: "
    "(1) What they studied at Cornell (major, minor, honors), (2) Research labs and "
...
SEARCH_VERTICALS = [
    "Harvard Medical School Cornell alumni",
    "Columbia Vagelos College of Physicians Cornell alumni",
    "Penn Perelman School of Medicine Cornell alumni",
    "Johns Hopkins School of Medicine Cornell alumni",
    "Stanford School of Medicine Cornell alumni",
    "Yale School of Medicine Cornell alumni",
    "UCSF School of Medicine Cornell alumni",
    ...
    "Icahn School of Medicine at Mount Sinai Cornell alumni",
]
...
VERTICAL_TO_DB_KEYWORDS = {
    "Harvard Medical School Cornell alumni": ["harvard"],
    "Columbia Vagelos College of Physicians Cornell alumni": ["columbia", "vagelos"],
    "Penn Perelman School of Medicine Cornell alumni": ["penn", "perelman", "pennsylvania"],
    "Johns Hopkins School of Medicine Cornell alumni": ["johns hopkins", "hopkins"],
    "Stanford School of Medicine Cornell alumni": ["stanford"],
    "Yale School of Medicine Cornell alumni": ["yale"],
    "UCSF School of Medicine Cornell alumni": ["ucsf"],
    ...
    "Icahn School of Medicine at Mount Sinai Cornell alumni": ["sinai", "icahn", "mount sinai"],
}
...
def pick_verticals_for_run(db_path: str, count: int = 3) -> list[str]:
    ...
    for vertical in SEARCH_VERTICALS:
        keywords = VERTICAL_TO_DB_KEYWORDS.get(vertical, [vertical.split(" Cornell")[0].split()[0].lower()])
    ...
    if scored and scored[0][0] > 0:
        picked.append(
            "Cornell University pre-med alumni who matched into top medical schools — "
            "search Match Day announcements, Cornell alumni magazine, Cornell pre-med advising spotlights, "
...
def run_search(num_leads: int = 10, max_workers: int = 4, db_path: str = "cornell_med.db",
               region: str = "United States", dry_run: bool = False):
    """Run the full discovery -> verification -> import pipeline."""
    init_db(db_path)

    logger.info("Starting Cornell med school search: %d leads, %d workers", num_leads, max_workers)
...
def main():
    parser = argparse.ArgumentParser(description="Cornell Med School Finder")
    ...
    parser.add_argument("--db-path", default="cornell_med.db", help="SQLite database path")
...
def _commit_and_push(db_path: str, imported: int):
    ...
    try:
        ...
        msg = f"Add {imported} new Cornell med school leads"
        subprocess.run(
            ["git", "commit", "-m", msg],
